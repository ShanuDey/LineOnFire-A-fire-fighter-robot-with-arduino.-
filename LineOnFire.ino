 ...
 * 
 * For more details
 * visit website : www.techreboot.tk
 * Thank you.
 */
...
#define ldrPin 10 //set ldr pin for the arduino
#define relayPin 11 //set the pin for relay input
...
void setup() {
  ...
  pinMode(ldrPin,INPUT); //LDR pin is defined as the input pin
  ...
  digitalWrite(relayPin, LOW); //Relay pin is set LOW for the starting
}

void loop() {
 //Serial.println(digitalRead(ldrPin));
 Candle();  //Delete this line if you only want to run the line follower robot, enable this line to sense the candle and extinguish that as well as following the line
...
void Candle()
{
  if(digitalRead(ldrPin)==0)//Candle spotted, Change this '0' to '1' if your fan is moving at wrong time
  {
    digitalWrite(left_enable, LOW); // motors off
    digitalWrite(right_enable, LOW);  // motors off
